...
# Supports: Axis Bank, Bank of Baroda, Kotak Bank, SBI
# """
...
class AxisBankParser(CreditCardParser):
    ...
    def parse(self, pdf_path: str) -> CreditCardStatement:
        ...
        return CreditCardStatement(
            bank_name="Axis Bank", card_number=card_number, statement_date=statement_date,
            payment_due_date=payment_due_date, credit_limit=credit_limit, total_amount_due=total_amount_due,
            minimum_amount_due=minimum_amount_due, available_credit=available_credit,
            previous_balance=previous_balance, transactions=transactions, extraction_quality=quality
        )
        ...
        for match in matches:
            ...
            if amount:
                ...
                transactions.append(Transaction(
                    ...
                    type=match[3].lower(), category=self.categorize_transaction(description)
                ))
        return transactions
...
class BobParser(CreditCardParser):
    ...
    def parse(self, pdf_path: str) -> CreditCardStatement:
        ...
        card_number = self.extract_field(text, [r'Card No\.:\s*XXXXXX\*{6}(\d{4})'])
        statement_date = self.extract_field(text, [r'Statement Date:\s*(\d{2}/\d{2}/\d{4})'])
        payment_due_date = self.extract_field(text, [r'Payment Due Date\s*(\d{2}/\d{2}/\d{4})'])
        ...
        return CreditCardStatement(
            bank_name="Bank of Baroda", card_number=card_number, statement_date=statement_date,
            payment_due_date=payment_due_date, credit_limit=credit_limit, total_amount_due=total_amount_due,
            minimum_amount_due=minimum_amount_due, available_credit=available_credit,
            previous_balance=previous_balance, transactions=transactions, extraction_quality=quality
        )
...
class KotakParser(CreditCardParser):
    ...
    def parse(self, pdf_path: str) -> CreditCardStatement:
        ...
        card_number = self.extract_field(text, [r'Primary Card Number\s+(\d{4}\s+XXXX\s+XXXX\s+\d{4})'])
        statement_date = self.extract_field(text, [r'Statement Date\s+(\d{2}-\w{3}-\d{4})'])
        payment_due_date = self.extract_field(text, [r'Remember to pay by\s+(\d{2}-\w{3}-\d{4})'])
        credit_limit = self.extract_field(text, [r'Total Credit Limit \(incl\. cash\):\s+Rs\.\s*([\d,]+\.\d{2})'], self.clean_amount)
        total_amount_due = self.extract_field(text, [r'Total Amount Due \(TAD\)\s+Rs\.\s*([\d,]+\.\d{2})'], self.clean_amount)
        minimum_amount_due = self.extract_field(text, [r'Minimum Amount Due \(MAD\)\s+Rs\.\s*([\d,]+\.\d{2})'], self.clean_amount)
        available_credit = self.extract_field(text, [r'Available Credit Limit:\s+Rs\.\s*([\d,]+\.\d{2})'], self.clean_amount)
        ...
        return CreditCardStatement(
            bank_name="Kotak Mahindra Bank", card_number=card_number, statement_date=statement_date,
            payment_due_date=payment_due_date, credit_limit=credit_limit, total_amount_due=total_amount_due,
            minimum_amount_due=minimum_amount_due, available_credit=available_credit,
            previous_balance=previous_balance, transactions=transactions, extraction_quality=quality
        )
...
class SBIParser(CreditCardParser):
    ...
    def parse(self, pdf_path: str) -> CreditCardStatement:
        ...
        card_number = self.extract_field(text, [r'Credit Card Number\s+XXXX XXXX XXXX (XX\d{2})'])
        statement_date = self.extract_field(text, [r'Statement Date\s+(\d{2} \w{3} \d{4})'])
        ...
        minimum_amount_due = self.extract_field(text, [r'\*\*Minimum Amount Due\s+\( \)\s*([\d,]+\.\d{2})'], self.clean_amount)
        available_credit = self.extract_field(text, [r'Available Credit Limit\s+\( \)\s*([\d,]+\.\d{2})'], self.clean_amount)
        ...
        return CreditCardStatement(
            bank_name="SBI Card", card_number=card_number, statement_date=statement_date,
            payment_due_date=payment_due_date, credit_limit=credit_limit, total_amount_due=total_amount_due,
            minimum_amount_due=minimum_amount_due, available_credit=available_credit,
            previous_balance=previous_balance, transactions=transactions, extraction_quality=quality
        )
...
class YesBankParser(CreditCardParser):
    ...
    def parse(self, pdf_path: str) -> CreditCardStatement:
        ...
        card_number = self.extract_field(text, [r'Statement for YES BANK Card Number\s+(\d{4}X+\d{4})'])
        statement_date = self.extract_field(text, [r'Statement Date:\s*(\d{2}/\d{2}/\d{4})'])
        payment_due_date = self.extract_field(text, [r'Payment Due Date:\s*(\d{2}/\d{2}/\d{4})'])
        credit_limit = self.extract_field(text, [r'Credit Limit:\s*Rs\.\s*([\d,]+\.\d{2})'], self.clean_amount)
        total_amount_due = self.extract_field(text, [r'Total Amount Due:\s*Rs\.\s*([\d,]+\.\d{2})'], self.clean_amount)
        minimum_amount_due = self.extract_field(text, [r'Minimum Amount Due:\s*Rs\.\s*([\d,]+\.\d{2})'], self.clean_amount)
        available_credit = self.extract_field(text, [r'Available Credit Limit:\s*Rs\.\s*([\d,]+\.\d{2})'], self.clean_amount)
        ...
        return CreditCardStatement(
            bank_name="YES Bank", card_number=card_number, statement_date=statement_date,
            payment_due_date=payment_due_date, credit_limit=credit_limit, total_amount_due=total_amount_due,
            minimum_amount_due=minimum_amount_due, available_credit=available_credit,
            previous_balance=previous_balance, transactions=transactions, extraction_quality=quality
        )
        ...
        if transaction_block_match:
            ...
            for match in matches:
                ...
                if amount:
                    ...
                    transactions.append(Transaction(
                        date=match[0].strip(), description=description, amount=amount,
                        type='debit', category=self.categorize_transaction(description)
                    ))
        return transactions
